...
const letterblock = {
  ...
  a: [[0, 8], [0, 8], [0, 1, 6, 1], [0, 6, 2], [0, 2, 6], [2, 4, 2], [0, 1, 5, 1, 1]],
  ...
  t: [[0, 1, 1, 4], [2, 4], [6], [2, 4], [2, 4], [2, 4], [0, 1, 5]],
  u: [[0, 8], [0, 8], [2, 4, 2], [2, 4, 2], [2, 4, 2], [2, 4, 2], [0, 1, 5, 1, 1]],
...
function printPetals(index: number) {
  const compile = (str: string, middle: string) => colors.magenta(str) + middle + colors.magenta(str.split("").reverse().join("").replace(index > 5 ? /\//g : /\\/g, index > 5 ? "\\" : "/").replace(/>/g, "<"));
